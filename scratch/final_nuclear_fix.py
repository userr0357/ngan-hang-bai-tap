...
# 1. Fix the failing fetch block that crashes the script
old_fetch_block = """  // Populate submission formats from DB
  const selFormat = document.getElementById('field-submission');
  if (selFormat) {
    fetch('/api/submission-formats')
      .then(r => r.json())
      .then(data => {
        selFormat.innerHTML = '';
        data.forEach(fmt => {
          const o = document.createElement('option');
          o.value = fmt.TenDinhDang;
          // Beautify labels
      ...
      .catch(e => console.warn('Failed to load formats:', e));
  }"""

new_fetch_block = """  // Populate submission formats from DB (Safe version)
  const selFormat = document.getElementById('field-submission');
  if (selFormat) {
    try {
      fetch('/api/submission-formats')
        ...
        .catch(e => {
          console.warn('Fallback to default formats due to API error');
        });
    } catch (err) {
      console.error('Submission formats fetch crashed:', err);
    }
  }"""

content = content.replace(old_fetch_block, new_fetch_block)

# 2. Define showExercise at the top of the file (after state definition)
# I'll search for the first occurrence of "const state ="
show_exercise_code = """
...
# Insert the function at the top (after state)
import re
content = re.sub(r"const state = \{.*?\};", lambda m: m.group(0) + show_exercise_code, content, flags=re.DOTALL)
